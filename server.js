...
app.use('/resources/:type/:file', function(req, res){
  ...
  let options = {
      root: __dirname + '/resources/'+filetype
  };
  let mimetype = {
    'html' : 'text/html',
    'ico' : 'image/x-icon',
...
app.use('/', function(req, res){
  ...
  let htmlHead = ReactDOMServer.renderToStaticMarkup(
    head(
     null,
     title({dangerouslySetInnerHTML:{__html: "歡迎光臨2017世大運"}}),
     meta({charSet: "utf-8"}),
	 meta({name: "title", content: "歡迎光臨2017世大運"}),
     meta({name: "description", content: "台北2017世大運，你有親臨現場為選手們喝采嗎?來為自己的到場支持、集章留念吧~"}),
